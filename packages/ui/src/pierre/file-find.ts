...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("palimpsest-find")
  api.delete("palimpsest-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: FileFindHit[], currentIndex: number) => {
    ...
    if (active) api.set("palimpsest-find-current", new Highlight(active))

    const rest = ranges.flatMap((hit, i) => (i === currentIndex ? [] : [hit.range]))
    if (rest.length > 0) api.set("palimpsest-find", new Highlight(...rest))
    return true
  }
