...
export namespace Domain {
  ...
  function fromProject(row: typeof ProjectTable.$inferSelect): Project {
    return {
      id: row.id,
  ...
  function fromWorkspace(row: typeof WorkspaceTable.$inferSelect): Workspace {
    return {
      id: row.id,
  ...
  function node(id: string) {
    const row = Database.use((db) => db.select().from(NodeTable).where(eq(NodeTable.id, id)).get())
    if (!row) throw new EntityNotFoundError({ entity: "node", id })
    return row
  }

  function run(id: string) {
    const row = Database.use((db) => db.select().from(RunTable).where(eq(RunTable.id, id)).get())
    if (!row) throw new EntityNotFoundError({ entity: "run", id })
    return row
  }

  function artifact(id: string) {
    const row = Database.use((db) => db.select().from(ArtifactTable).where(eq(ArtifactTable.id, id)).get())
    if (!row) throw new EntityNotFoundError({ entity: "artifact", id })
    return row
  }

  function decision(id: string) {
    const row = Database.use((db) => db.select().from(DecisionTable).where(eq(DecisionTable.id, id)).get())
  ...
  function kinds(projectID: string) {
    ...
    const row = Database.use((db) =>
      db.select().from(ProjectTaxonomyTable).where(eq(ProjectTaxonomyTable.project_id, projectID)).get(),
    )
  ...
  export const removeNode = fn(Identifier.schema("node"), async (id) => {
    ...
    Database.use((db) => db.delete(NodeTable).where(eq(NodeTable.id, id)).run())
    return info
  })
  ...
  export const getEdge = fn(Identifier.schema("edge"), async (id) => {
    const row = Database.use((db) => db.select().from(EdgeTable).where(eq(EdgeTable.id, id)).get())
  ...
  export const updateEdge = fn(
    ...
    async (input) => {
      const row = Database.use((db) => db.select().from(EdgeTable).where(eq(EdgeTable.id, input.id)).get())
  ...
  export const removeEdge = fn(Identifier.schema("edge"), async (id) => {
    ...
    Database.use((db) => db.delete(EdgeTable).where(eq(EdgeTable.id, id)).run())
    return info
  })
  ...
  export const removeRun = fn(Identifier.schema("run"), async (id) => {
    ...
    Database.use((db) => db.delete(RunTable).where(eq(RunTable.id, id)).run())
    return info
  })
  ...
  export const removeArtifact = fn(Identifier.schema("artifact"), async (id) => {
    ...
    Database.use((db) => db.delete(ArtifactTable).where(eq(ArtifactTable.id, id)).run())
    return info
  })
  ...
  export const removeDecision = fn(Identifier.schema("decision"), async (id) => {
    ...
    Database.use((db) => db.delete(DecisionTable).where(eq(DecisionTable.id, id)).run())
    return info
  })
  ...
  export const summary = fn(z.string(), async (projectID) => {
    ...
    const workspaces = await listWorkspaces.force(projectID)
  ...
  export const getProject = fn(z.string(), async (projectID) => {
    return fromProject(project(projectID))
  })

  export const listWorkspaces = fn(z.string(), async (projectID) => {
    project(projectID)
    const rows = Database.use((db) =>
      db
        .select()
  ...
  export const context = fn(z.string(), async (projectID) => {
    return Context.parse({
      project: await getProject.force(projectID),
      workspaces: await listWorkspaces.force(projectID),
      taxonomy: await taxonomy.force(projectID),
      summary: await summary.force(projectID),
    })
  })
  ...
  function proposal(id: string) {
    const row = Database.use((db) => db.select().from(ProposalTable).where(eq(ProposalTable.id, id)).get())
    if (!row) throw new EntityNotFoundError({ entity: "proposal", id })
    return row
  }

  function review(id: string) {
    const row = Database.use((db) => db.select().from(ReviewTable).where(eq(ReviewTable.id, id)).get())
    if (!row) throw new EntityNotFoundError({ entity: "review", id })
    return row
  }

  function commit(id: string) {
    const row = Database.use((db) => db.select().from(CommitTable).where(eq(CommitTable.id, id)).get())
  ...
  function apply(projectID: string, raw: Change) {
    ...
    switch (raw.op) {
      ...
      case "delete_node": {
        ...
        Database.use((db) => db.delete(NodeTable).where(eq(NodeTable.id, raw.id)).run())
        return
      }
      case "create_edge": {
      ...
      case "update_edge": {
        const row = Database.use((db) => db.select().from(EdgeTable).where(eq(EdgeTable.id, raw.id)).get())
      ...
      case "delete_edge": {
        ...
        if (!row) throw new EntityNotFoundError({ entity: "edge", id: raw.id })
        check(row, "edge", raw.id, projectID)
        Database.use((db) => db.delete(EdgeTable).where(eq(EdgeTable.id, raw.id)).run())
        return
      }
      case "create_run": {
      ...
      case "delete_run": {
        ...
        Database.use((db) => db.delete(RunTable).where(eq(RunTable.id, raw.id)).run())
        return
      }
      ...
      case "delete_artifact": {
        ...
        Database.use((db) => db.delete(ArtifactTable).where(eq(ArtifactTable.id, raw.id)).run())
        return
      }
      ...
      case "delete_decision": {
        ...
        Database.use((db) => db.delete(DecisionTable).where(eq(DecisionTable.id, raw.id)).run())
      }
    }
  }
