...
import { selectionFromLines, type FileSelection, type SelectedLineRange } from "./types"
import { showToast } from "@palimpsest/ui/toast"
import { findLast } from "@palimpsest/shared/array"
import { extractPromptFromParts } from "./prompt-input/extract-prompt"
import type { UserMessage } from "@palimpsest/sdk/v2/client"
import { usePluginWebHost, type PluginWebHostCommandEntry } from "../../host-web"
import { useSessionID } from "./session-id"
import { canAddSelectionContext } from "./session-command-helpers"
import { DialogFork } from "./dialog-fork"
import { DialogSelectMcp } from "./dialog-select-mcp"
...
const withCategory = (category: string) => {
  return (option: Omit<PluginWebHostCommandEntry, "category">): PluginWebHostCommandEntry => ({
    ...option,
    category,
  })
}

export type SessionCommandSlots = {
  selectFile: Component<{ onOpenFile: () => void }>
  selectModel: Component
}

export type SessionCommandTerminal = {
  all(): unknown[]
  new: () => void
}

export const useSessionCommands = (actions: SessionCommandContext & {
  slots: SessionCommandSlots
  terminal: SessionCommandTerminal
}) => {
  ...
  const terminal = actions.terminal
  const layout = host.layout()
  const params = useSessionID()
  const navigate = useNavigate()
  const DialogSelectFile = actions.slots.selectFile
  const DialogSelectModel = actions.slots.selectModel
