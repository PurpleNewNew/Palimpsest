import { useFilteredList } from "@palimpsest/ui/hooks"
...
import { DockShellForm, DockTray } from "@palimpsest/ui/dock-surface"
...
import { RadioGroup } from "@palimpsest/ui/radio-group"
import { useDialog } from "@palimpsest/ui/context/dialog"
import { ModelSelectorPopover, type ModelSelectorSlots } from "../dialog-select-model"
import { DialogSelectModelUnpaid, type DialogSelectModelUnpaidSlots } from "../dialog-select-model-unpaid"
...
import { createPromptAttachments, ACCEPTED_FILE_TYPES } from "./attachments"
import {
...
export type PromptInputIdeaDialogProps = {
  onCancel: () => void
  onSubmit: (idea: string) => void
}

export type PromptInputIdeaAction = {
  id: string
  display: string
  agent: string
  mentions: readonly string[]
  buildPrompt: (idea: string) => string
  dialog: Component<PromptInputIdeaDialogProps>
}

export type PromptInputSlots = {
  modelSelector: ModelSelectorSlots
  unpaidModelSelector: DialogSelectModelUnpaidSlots
  imagePreview: Component<{ src: string; alt: string }>
  idea?: PromptInputIdeaAction
}
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const files = host.file()
  const prompt = host.prompt()
  const layout = host.layout()
  const comments = host.comments()
  ...
  const ImagePreview = props.slots.imagePreview
  const keybind = (id: string) => command.keybind(id) ?? ""
  ...
  const startIdeaWorkflow = async (idea: string) => {
    ...
    const action = props.slots.idea
    if (!action) return

    const text = action.buildPrompt(idea)
  ...
  const handleAtSelect = (option: AtOption | undefined) => {
    ...
    if (option.type === "action") {
      closePopover()
      const action = props.slots.idea
      if (action && option.name === action.id) {
        const DialogIdea = action.dialog
        dialog.show(() => (
          <DialogIdea
            onCancel={() => {}}
            onSubmit={(idea) =>
                  ...
                  fallback={
                    ...
                    >
                      <Button
                        ...
                        onClick={() => dialog.show(() => <DialogSelectModelUnpaid slots={props.slots.unpaidModelSelector} />)}
                      >
                ...
                >
                  ...
                  >
                    <ModelSelectorPopover
                      slots={props.slots.modelSelector}
                      triggerAs={Button}
                      triggerProps={{
                        variant: "ghost",
                        size: "normal",
              ...
              <RadioGroup
                ...
                label={(mode) => (
                  <TooltipKeybind
                    ...
                    keybind={keybind(mode === "shell" ? "prompt.mode.shell" : "prompt.mode.normal")}
