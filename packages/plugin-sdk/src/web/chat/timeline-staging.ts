...
/**
 * Defer-mounts small timeline windows so revealing older turns does
 * not block first paint with a large DOM mount.
 *
 * Once staging completes for a session it never re-stages — backfill
 * and new messages render immediately.
 *
 * Migrated from `apps/web/src/pages/session/timeline-staging.ts` in
 * Phase 2.3 of the host context promotion. No host dependencies; only
 * the imported `UserMessage` type was retargeted from
 * `@palimpsest/sdk/v2` to `@palimpsest/sdk/v2/client` to keep
 * plugin-sdk free of the SDK's node-only server module.
 */
