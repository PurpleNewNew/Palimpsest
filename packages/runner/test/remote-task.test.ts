...
EOF`)
  ...
  test("generates short unique screen session names", () => {
    ...
    expect(a.startsWith("palimpsest-")).toBeTrue()
    expect(b.startsWith("palimpsest-")).toBeTrue()
    expect(a).not.toBe(b)
    expect(a).toMatch(/^palimpsest-\d{13}-[a-f0-9]{6}$/)
    expect(b).toMatch(/^palimpsest-\d{13}-[a-f0-9]{6}$/)
  })
})
