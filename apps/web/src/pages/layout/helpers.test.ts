...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("palimpsest://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("palimpsest://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("palimpsest://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("palimpsest://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("palimpsest://open-project")).toBeUndefined()
    expect(parseDeepLink("palimpsest://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "palimpsest://open-project?directory=/a",
      "palimpsest://other?directory=/b",
      "palimpsest://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("palimpsest://new-session?directory=/tmp/demo")).toEqual({
      directory: "/tmp/demo",
    })
    expect(parseNewSessionDeepLink("palimpsest://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("palimpsest://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("palimpsest://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "palimpsest://new-session?directory=/a",
      "palimpsest://open-project?directory=/b",
      "palimpsest://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __PALIMPSEST__: {
        deepLinks: ["palimpsest://open-project?directory=/a"],
      },
    } as unknown as Window & { __PALIMPSEST__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["palimpsest://open-project?directory=/a"])
