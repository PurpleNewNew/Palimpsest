...
export function DomainSidebarOverview(props: {
  ...
  const cards = createMemo<Array<{ id: string; label: string; value: number; href: string }>>(() => {
    ...
    const coreTabs = (data()?.shell?.workspaceTabs ?? [])
      .filter((tab) => tab.kind === "core")
      .slice()
      .sort((a, b) => b.priority - a.priority)
    const coreCards = coreTabs.map((tab) => ({
      id: tab.id,
      label: tab.title,
    ...
    return [
      ...coreCards,
      { id: "commits", label: "Commits", value: summary.commits, href: `${base}/commits` },
    ]
  })
                  ...
                  <For each={cards()}>
                    {(item) => (
                      <A
                        ...
                        data-component="domain-overview-tile"
                        data-tab-id={item.id}
                      >
