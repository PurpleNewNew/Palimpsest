import { AppIcon } from "@palimpsest/ui/app-icon"
...
const detectOS = (platform: ReturnType<typeof usePlatform>): OS => {
  if (platform.platform === "native" && platform.os) return platform.os
...
function useSessionShare(args: {
  ...
  const shareSession = () => {
    ...
    if (!id || state.share) return
    setState("share", true)
    args.globalSDK.client.session
      .share({ sessionID: id, directory: args.projectDirectory() })
      .catch((err) => {
  ...
  const unshareSession = () => {
    const id = args.sessionID()
    if (!id || state.unshare) return
    setState("unshare", true)
    args.globalSDK.client.session
      .unshare({ sessionID: id, directory: args.projectDirectory() })
      .catch((err) => {
  ...
  const copyLink = (onError: (err: unknown) => void) => {
    const url = shareUrl()
    if (!url) return
    navigator.clipboard
      .writeText(url)
...
export function SessionHeader() {
  ...
  const project = createMemo(() => {
    const value = dir()
    if (!value) return
    return layout.projects.list().find((p) => p.worktree === value || p.sandboxes?.includes(value))
  })
  ...
  const view = createMemo(() => layout.view(`${params.dir}${params.id ? "/" + params.id : ""}`))
  ...
  createEffect(() => {
    if (platform.platform !== "native") return
    ...
    void Promise.all(
      ...
      setExists(Object.fromEntries(list) as Partial<Record<OpenApp, boolean>>)
    })
  })
  ...
  const canOpen = createMemo(() => platform.platform === "native" && !!platform.openPath && server.isLocal())
  const current = createMemo(
    () =>
      options().find((item) => item.id === prefs.app) ??
      options()[0] ??
      ({ id: "finder", label: fileManager().label, icon: fileManager().icon } as const),
  )
  const opening = createMemo(() => req.app !== undefined)
  ...
  const openDir = (app: OpenApp) => {
    ...
    const value = dir()
    if (!value) return
    const item = options().find((entry) => entry.id === app)
    const openWith = item && "openWith" in item ? item.openWith : undefined
    setReq("app", app)
    platform
      .openPath(value, openWith)
      .catch((err: unknown) => showRequestError(language, err))
      .finally(() => {
        setReq("app", undefined)
      })
  }
                      ...
                      <div class="flex h-[24px] box-border items-center rounded-md border border-border-weak-base bg-surface-panel overflow-hidden">
                        ...
                        >
                          <div class="flex size-5 shrink-0 items-center justify-center">
                            <Show when={opening()} fallback={<AppIcon id={current().icon} class={openIconSize(current().icon)} />}>
                                ...
                                >
                                  <For each={options()}>
                                    {(item) => (
                                      ...
                                      >
                                        <div class="flex size-5 shrink-0 items-center justify-center">
                                          <AppIcon id={item.icon} class={openIconSize(item.icon)} />
                                        </div>
                  ...
                  <Show when={share.shareUrl()} fallback={<div aria-hidden="true" />}>
                    ...
                    >
                      <IconButton
                        ...
                        onClick={() => share.copyLink((err) => showRequestError(language, err))}
                        disabled={share.state.unshare}
                        aria-label={
                          share.state.copied
                            ? language.t("session.share.copy.copied")
