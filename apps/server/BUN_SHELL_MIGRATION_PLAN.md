...
Replace runtime Bun shell template-tag usage in `apps/server/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `apps/server/src` is removed except:
