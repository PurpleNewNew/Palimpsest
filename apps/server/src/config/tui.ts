...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.PALIMPSEST_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.PALIMPSEST_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.PALIMPSEST_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".palimpsest") && dir !== Flag.PALIMPSEST_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old palimpsest.json shape) still get their settings applied.
    const normalized = (() => {
