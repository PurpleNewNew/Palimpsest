...
function globalFiles() {
  ...
  if (Flag.PALIMPSEST_CONFIG_DIR) {
    files.push(path.join(Flag.PALIMPSEST_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.PALIMPSEST_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.PALIMPSEST_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no PALIMPSEST_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.PALIMPSEST_CONFIG_DIR, Flag.PALIMPSEST_CONFIG_DIR).catch(() => [])
}
