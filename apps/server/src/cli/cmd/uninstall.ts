...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall Palimpsest and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Palimpsest")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g palimpsest-ai",
      pnpm: "pnpm uninstall -g palimpsest-ai",
      bun: "bun remove -g palimpsest-ai",
      yarn: "yarn global remove palimpsest-ai",
      brew: "brew uninstall palimpsest",
      choco: "choco uninstall palimpsest",
      scoop: "scoop uninstall palimpsest",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall palimpsest -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Palimpsest!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# palimpsest") || content.includes(".palimpsest/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".palimpsest/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".palimpsest/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".palimpsest"))
    ) {
      continue
    }
