...
import type { PermissionRequest } from "@palimpsest/sdk/v2"
          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"This will allow " + props.request.permission + " until Palimpsest is restarted."} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>This will allow the following patterns until Palimpsest is restarted</text>
                  <box>
      ...
      <Match when={store.stage === "permission"}>
        {(() => {
          const info = () => {
            ...
            if (permission === "huggingface_search") {
              const query = typeof data.query === "string" ? data.query : ""
              return {
                icon: "◈",
                title: `Hugging Face Search "${query}"`,
                body: (
                  <Show when={query}>
                    <box paddingLeft={1}>
                      <text fg={theme.textMuted}>{"Query: " + query}</text>
            ...
            if (permission === "modelscope_search") {
              const query = typeof data.query === "string" ? data.query : ""
              return {
                icon: "◈",
                title: `ModelScope Search "${query}"`,
                body: (
                  <Show when={query}>
                    <box paddingLeft={1}>
                      <text fg={theme.textMuted}>{"Query: " + query}</text>
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell Palimpsest what to do differently</text>
        </box>
      </box>
      <box
