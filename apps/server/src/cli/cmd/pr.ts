...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run Palimpsest",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await $`palimpsest import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting Palimpsest...")
        UI.println()

        // Launch Palimpsest with session ID if available
        const { spawn } = await import("child_process")
        const palimpsestArgs = sessionId ? ["-s", sessionId] : []
        const palimpsestProcess = spawn("palimpsest", palimpsestArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          palimpsestProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`palimpsest exited with code ${code}`))
          })
          palimpsestProcess.on("error", reject)
        })
      },
    })
  },
