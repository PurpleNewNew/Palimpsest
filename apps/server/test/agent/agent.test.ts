...
test("plan agent denies edits except .palimpsest/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("edit", ".palimpsest/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".palimpsest", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  const home = process.env.PALIMPSEST_TEST_HOME
  process.env.PALIMPSEST_TEST_HOME = tmp.path
