...
describe("plugin host API (Stage B)", () => {
  ...
  test("security-audit plugin registers audit tools via host.tools.register", () =>
    serverTest(async ({ dirs }) => {
      ...
      // security-audit proposal tools (auto-prefixed)
      ...
      expect(ids).toContain("security_overview")
      expect(ids).toContain("security_findings")
      expect(ids).toContain("security_graph")
      expect(ids).toContain("evidence_add")
      expect(ids).toContain("surface_add")
      expect(ids).toContain("control_add")
      expect(ids).toContain("assumption_add")
      // host-provided domain editing / proposal revise tools
      expect(ids).toContain("node_update")
      expect(ids).toContain("node_delete")
      expect(ids).toContain("edge_update")
      expect(ids).toContain("edge_delete")
      expect(ids).toContain("proposal_revise")
    }))
})
