...
async function writeManagedSettings(settings: object, filename = "palimpsest.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "palimpsest.json") {
...
test("ignores legacy UI shell keys in palimpsest config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://palimpsest.dev/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "palimpsest.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "palimpsest.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "palimpsest.json")).href
...
test("local .palimpsest config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
describe("deduplicatePlugins", () => {
  ...
  test("local plugin directory overrides global palimpsest.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const metaDir = path.join(projectDir, ".palimpsest")
...
describe("PALIMPSEST_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .palimpsest/ directories when flag is set", async () => {
    const originalEnv = process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"]
    process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .palimpsest should NOT be in directories list
