...
const managedConfigDir = process.env.PALIMPSEST_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "openresearch.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openresearch.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "openresearch.jsonc"),
        `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openresearch.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openresearch.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .openresearch directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
test("loads agents from .openresearch/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openresearch")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openresearch/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openresearch")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openresearch/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openresearch")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only PALIMPSEST_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "openresearch.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openresearch config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".openresearch")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .openresearch config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "openresearch.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openresearch config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "openresearch.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets PALIMPSEST_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openresearch config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openresearch directory config enables it
      const opencodeDir = path.join(dir, ".openresearch")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "openresearch.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.openresearch/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.openresearch/plugin/oh-my-opencode.js")
  })
  ...
  test("local plugin directory overrides global openresearch.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const opencodeDir = path.join(projectDir, ".openresearch")
...
describe("PALIMPSEST_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"]
    process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openresearch/ directories when flag is set", async () => {
    const originalEnv = process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"]
    process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openresearch directory with a command
          const opencodeDir = path.join(dir, ".openresearch", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .openresearch should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PALIMPSEST_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["PALIMPSEST_CONFIG_DIR"]
      process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["PALIMPSEST_CONFIG_DIR"]
      } else {
        process.env["PALIMPSEST_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("PALIMPSEST_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PALIMPSEST_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["PALIMPSEST_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["PALIMPSEST_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from PALIMPSEST_CONFIG_DIR, not project
...
describe("PALIMPSEST_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in PALIMPSEST_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PALIMPSEST_CONFIG_CONTENT"]
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["PALIMPSEST_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["PALIMPSEST_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in PALIMPSEST_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PALIMPSEST_CONFIG_CONTENT"]
