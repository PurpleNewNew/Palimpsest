...
export default function Literature(): JSX.Element {
  ...
  const [bundle] = createResource(async () => {
    ...
    if (!researchProject) return { project, researchProject: undefined, articles: [] }
    const articles = await research.articles(researchProject.research_project_id).catch(() => [])
    return { project, researchProject, articles }
  })

  const sortedArticles = createMemo(() =>
    (bundle()?.articles ?? []).slice().sort((a, b) => (a.title ?? a.filename).localeCompare(b.title ?? b.filename)),
  )
          ...
          <>
            <div class="grid gap-4 md:grid-cols-3">
              ...
              <div class="rounded-2xl bg-surface-raised-base px-4 py-4">
                ...
                <div class="mt-2 text-16-medium text-text-strong">{sortedArticles().length}</div>
              </div>
            </div>

            <section class="rounded-2xl bg-surface-raised-base px-4 py-4" data-component="research-literature-list">
              <div class="flex items-center justify-between">
                <div class="text-11-medium uppercase tracking-wide text-text-weak">Article Corpus</div>
                <span class="text-11-regular text-text-weak">{sortedArticles().length} items</span>
              </div>
                  ...
                  {(article) => (
                    <div class="rounded-xl bg-background-base px-3 py-3">
                      <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                          <div class="text-13-medium text-text-strong">{article.title?.trim() || article.filename}</div>
                          <div class="mt-1 break-all text-11-regular text-text-weak">{article.filename}</div>
                        </div>
