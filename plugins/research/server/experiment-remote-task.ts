...
import { RemoteTaskTable } from "./research-schema"

type Row = typeof RemoteTaskTable.$inferSelect
...
function row(input: Lookup): Row | undefined {
  if (input.taskId) {
...
export namespace ExperimentRemoteTask {
  ...
  export function listByExp(expId: string): Row[] {
    return bridge().db.use((db: any) =>
      db.select().from(RemoteTaskTable).where(eq(RemoteTaskTable.exp_id, expId)).all(),
    ) as Row[]
  }

  export function listActive(): Row[] {
    return bridge().db.use((db: any) =>
      db
        .select()
        .from(RemoteTaskTable)
  ...
  export function latest(expId: string): Row | undefined {
    return [...listByExp(expId)].sort((a, b) => b.time_updated - a.time_updated)[0]
  }

  export function deleteByExp(expId: string) {
    bridge().db.use((db: any) => db.delete(RemoteTaskTable).where(eq(RemoteTaskTable.exp_id, expId)).run())
  }
}
